...
public class Country {
	...
	private double STD(int[] values) {
		...
		return Math.sqrt( (double) squaredsum/values.length - Math.pow((double) sum/values.length,2));
	...
	public double getCorrelation() {
		if (this.infections.size()>0) {
			...
			double mean1 =(double) this.getInfectionCount()/ (double) this.infections.size();
			double mean2 =4;
			double s1 = STD(val1);
			double s2 = STD(val2);
